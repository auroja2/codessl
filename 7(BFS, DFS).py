...
# DFS Recursive function with Preorder, Inorder, and Postorder options
def dfs_recursive(graph, node, traversal_type):
    ...
    def _dfs_inorder(node):
        if node not in visited:
            visited.add(node)
            if len(graph[node]) > 0:
                _dfs_inorder(graph[node][0])  # Left child (if exists)
            print(node, end=' ')
            if len(graph[node]) > 1:
                _dfs_inorder(graph[node][1])  # Right child (if exists)
    ...
    # Perform the DFS based on the type selected
    ...
    while True:
        ...
        if choice == 1:
            ...
            try:
                ...
                # Perform DFS with an initial traversal (Preorder)
                dfs_recursive(graph, start_node, 'preorder')
                print()
                
                # Ask for DFS traversal type
